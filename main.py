...
def main():
    ...
    penv.activate_tree(tree=tree)

    # Create the PyRender scene for colors. Tree gets recolored in Tree() instantiation and exported to obj path
    tmesh = RenderScene.load_mesh(tree.obj_mesh_path)
    pyr_scene = RenderScene(mesh=tmesh)
    ...
    pyr_scene.add_camera(camera=robot.sensors['tof0'], extrinsics=camera_extrinsics, mode='depth', camera_name='tof0')
